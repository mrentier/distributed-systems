...
The above example of 'the sum of unique numbers' demonstrates a rather fundamental issue in distributed systems: how to reach consensus among several nodes when different events occur across the nodes. Broadly speaking, there are two possible ways of dealing with such differences: strongly consistent replication or optimistic replication.
...
The 'sum of unique numbers' example utilizes a data-structure called a grow-only set. A grow-only set is one of a group of data-structures named Conflict-free Replicated Data Types (CRDTs). CRDTs fall in the optimistic replication model. Conflict-free is a vague description, but it comes down to the following: <em>we are operating on data structures that don't require exclusive write access and are 1) able to detect concurrent updates and 2) perform deterministic, automatic conflict resolution</em>. This doesn't imply an absence of conflict, but we are able to always determine the output up front, based on metadata contained within the structure itself. The core CRDTs are counters, registers and sets, but from them we can compose more advanced ones like maps or even JSON.
